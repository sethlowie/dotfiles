...
if use_coc then
    ...
    silentRemap('cr', ':CocCommand rls.run')
    silentRemap('gb', ':GBranches')
    silentRemap('gd', '<Plug>(coc-definition)')
    silentRemap('gy', '<Plug>(coc-type-definition)')
    silentRemap('gi', '<Plug>(coc-implementation)')
    silentRemap('gr', '<Plug>(coc-references)')
    silentRemap('v', '<Plug>(coc-fix-current)')
    silentRemap('af', ':CocCommand eslint.executeAutoFix')
    vim.cmd("let g:coc_global_extensions = ['coc-tslint-plugin','coc-tsserver','coc-emmet','coc-css','coc-html','coc-json','coc-yank','coc-prettier','coc-fzf-preview','coc-rls','coc-eslint']")
else
